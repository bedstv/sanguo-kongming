...
function dataBuf(s){return Buffer.from(s.replace(/^data:image\/png;base64,/,''),'base64')}
function pngSizeFromDataUrl(s){const b=dataBuf(s);if(b.subarray(1,4).toString()!=='PNG')throw new Error('invalid PNG data');return [b.readUInt32BE(16),b.readUInt32BE(20)]}
...
const h=fs.readFileSync('golden-v11.html','utf8');for(const s of ['V11.6.1 Golden Battle','v110.css?v=11.6.1','v113-audio.css?v=11.6.1','goldenBattleV11.js?v=11.6.1','soundGateV11'])if(!h.includes(s))throw new Error(`golden-v11 missing ${s}`);
...
console.log('V11.6.1 Guan Yu sprite hotfix + iPhone composition + audio validation OK');
