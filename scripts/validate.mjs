...
const requiredV8=['GOAL_V8.md','docs/BATTLE_V8_REBUILD.md','src/v8/battleArt.js','src/v8/battleRenderer.js','src/v8/musicV8.js','src/v8/pixelUi.js','styles/v80.css'];
for(const f of requiredV8){if(!fs.existsSync(path.join(root,f)))bad(`V8 asset missing: ${f}`);else ok(`V8 asset: ${f}`)}

const battle=fs.readFileSync(path.join(root,'src/battle.js'),'utf8');
if(!battle.includes("from './v8/battleRenderer.js'"))bad('battle system is not using V8 renderer');else ok('V8 battle renderer wired');
const renderer=fs.readFileSync(path.join(root,'src/v8/battleRenderer.js'),'utf8');
if(!renderer.includes('v8-segbar')||!renderer.includes('battleSpriteData'))bad('V8 segmented bars or battle art missing');else ok('V8 unit presentation');
...
for(const need of ['v80.css','battleArt.js','battleRenderer.js','musicV8.js','pixelUi.js'])if(!sw.includes(need))bad(`V8 asset not precached: ${need}`);
