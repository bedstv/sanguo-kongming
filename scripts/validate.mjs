...
for(const [id,map] of Object.entries(MAPS)){
  if(map.rows.length!==map.height)bad(`${id}: height=${map.height}, rows=${map.rows.length}`);
  else ok(`${id}: row count`);
  for(let y=0;y<map.rows.length;y++){
    if(map.rows[y].length!==map.width)bad(`${id}: row ${y} width=${map.rows[y].length}, expected=${map.width}`);
  }
